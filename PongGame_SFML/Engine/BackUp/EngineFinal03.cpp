...
void Update(sf::RenderWindow* window, sf::Time* dt)
{
	...
	while (window->pollEvent(event))
	{
		...
		if (Begin == false)
		{
			...
			if (diffChosen == false && modeChosen == true)
			{
				switch (event.key.code)
				{
	...
	if (Play == true && Start == true)
	{
		...
		if (Medium)
		{
			...
			block1.setPosition(x, rand() % +300 + 100);
			block2.setPosition(rand() % 500 + 250, rand() % +300 + 100);
			block3.setPosition(rand() % 500 + 250, rand() % +300 + 100);
		}
		...
		AI(player2MovingUp, player2MovingDown, &bluePadSpd, dt); //TODO
		//cout << "BLue pad spd is: " << bluePadSpd << endl;
		/*
